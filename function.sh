...
usage_deploy_pkg() {
    ...
    echo '    -r <repo>          Repository package belongs to. [Options = alarm, core, extra or community]'
...
usage_deploy_img() {
    ...
    echo "    -d <device>        Device the image is for. [Default = rpi4. Options = rpi3, rpi4, oc2, vim1, bpir2, on2, vim3, rock64, rockpro64, rockpi4, sopine, pine64 and pinebook]"
    echo "    -a <arch>          Architecture. [Default = aarch64. Options = any, armv7h  or aarch64]"
...
#Start editing for armv7h BPIR2
create_rootfs_pkg() {
    ...
    if [[ "$ARCH" = "armv7h" ]]; then
     ...
     #http://mirror.archlinuxarm.org/armv7h/ 
     #http://mirror.archlinuxarm.org/armv7h/extra/389-ds-base-1.4.1.6-7-armv7h.pkg.tar.xz
      else
    ...
    basestrap -G -C $LIBDIR/pacman.conf.$ARCH $BUILDDIR/$ARCH base-devel archlinux-keyring
    
    #manjaro-arm-keyring

    # Enable cross architecture Chrooting
    if [[ "$ARCH" = "armv7h" ]]; then
    cp /usr/bin/qemu-arm-static $ROOTFS_IMG/rootfs_$ARCH/usr/bin/
     else
   ...
   $NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman-key --init 1> /dev/null 2>&1
    $NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman-key --populate archlinuxarm 1> /dev/null 2>&1
     msg "Copying Resolv Conf of $ARCH for $DEVICE..."
    sudo mv $ROOTFS_IMG/rootfs_$ARCH/etc/resolv.conf $ROOTFS_IMG/rootfs_$ARCH/etc/resolv.bak  
    sudo cp /etc/resolv.conf $ROOTFS_IMG/rootfs_$ARCH/etc
     $NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman -Sy --noconfirm
    $NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman -S archlinux-keyring --noconfirm
     else
...
create_rootfs_img() {
    ...
    if [["$ARCH" = "armv7h"]]; then 
    rm -rf $ROOTFS_IMG/ArchLinuxARM-$ARCH-latest.tar.gz*
    else
    ...
    cd $ROOTFS_IMG
     if [["$ARCH" = "armv7h"]]; then 
     wget -q --show-progress --progress=bar:force:noscroll http://gr.mirror.archlinuxarm.org/os/ArchLinuxARM-armv7-latest.tar.gz
     else
    ...
    #http://gr.mirror.archlinuxarm.org/os/ArchLinuxARM-armv7-latest.tar.gz
    fi
    #also fetch it, if it does not exist
    if [["$ARCH" = "armv7h"]]; then 
    if [ ! -f "$ROOTFS_IMG/ArchLinuxARM-$ARCH-latest.tar.gz" ]; then
    cd $ROOTFS_IMG
    wget -q --show-progress --progress=bar:force:noscroll http://gr.mirror.archlinuxarm.org/os/ArchLinuxARM-armv7-latest.tar.gz
    #http://gr.mirror.archlinuxarm.org/os/ArchLinuxARM-armv7-latest.tar.gz
    fi
    ...
    info "Extracting $ARCH rootfs..."
     if [["$ARCH" = "armv7h"]]; then 
     bsdtar -xpf $ROOTFS_IMG/ArchLinuxARM-$ARCH-latest.tar.gz -C $ROOTFS_IMG/rootfs_$ARCH     
     else
    ...
    elif [[ "$DEVICE" = "oc2" ]] || [[ "$DEVICE" = "vim1" ]]; then
...
create_rootfs_oem() {
    ...
    info "Extracting $ARCH rootfs..."
    if [[ "$DEVICE" = "bpir2" ]]; then 
    bsdtar -xpf $ROOTFS_IMG/ArchLinuxARM-armv7-latest.tar.gz -C $ROOTFS_IMG/rootfs_$ARCH
    else
    bsdtar -xpf $ROOTFS_IMG/Manjaro-ARM-$ARCH-latest.tar.gz -C $ROOTFS_IMG/rootfs_$ARCH
    fi 
    ...
    # Enable cross architecture Chrooting
      msg "Cross Compiling of $ARCH for $DEVICE..."
     if [[ "$DEVICE" = "bpir2" ]]; then
    cp /usr/bin/qemu-arm-static $ROOTFS_IMG/rootfs_$ARCH/usr/bin/
     else
    cp /usr/bin/qemu-aarch64-static $ROOTFS_IMG/rootfs_$ARCH/usr/bin/
    fi
   ...
   $NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman -Sy --noconfirm
    ...
    #$NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman-key --init
    #$NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman-key --populate archlinuxarm archlinux
    #$NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman-key --init
    #$NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman-key --populate archlinuxarm archlinux
    #$NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman-key --init 1> /dev/null 2>&1
    #$NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman-key --populate archlinuxarm archlinux manjaro manjaro-arm  1> /dev/null 2>&1
    #$NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman-key --populate
    #sudo pacman -Sy archlinux-keyring manjaro-keyring manjaro manjaro-arm
        
    msg "Installing packages for $EDITION edition on $DEVICE..."
    # Install device and editions specific packages
    mount -o bind /var/cache/manjaro-arm-tools/pkg/pkg-cache $ROOTFS_IMG/rootfs_$ARCH/var/cache/pacman/pkg
    $NSPAWN $ROOTFS_IMG/rootfs_$ARCH pacman -Syyu base $PKG_DEVICE $PKG_EDITION dialog --noconfirm
    # manjaro-arm-oem-install
    #Worked till here
    if [[ "$DEVICE" = "on2" ]] || [[ "$DEVICE" = "rpi4" ]] || [[ "$DEVICE" = "vim3" ]] || [[ "$DEVICE" = "vim1" ]]; then
    if [[ "$EDITION" = "kde" ]] || [[ "$EDITION" = "cubocore" ]]; then
...
#For BPI-R2 rootfs
    ...
    elif [[ "$DEVICE" = "rpi3" ]] || [[ "$DEVICE" = "rpi3-fta" ]] || [[ "$DEVICE" = "rpi4" ]]; then
        #partition with boot and root
        parted -s $LDEV mklabel msdos 1> /dev/null 2>&1
    ...
    #clean up
        ...
        partprobe $LDEV 1> /dev/null 2>&1
         ...
         ## For Khadas devices Vim1   
       elif [[ "$DEVICE" = "vim1" ]]; then 
        #partition with boot and root
        parted -s $LDEV mklabel msdos 1> /dev/null 2>&1
        #parted -s $LDEV mkpart primary fat32 0% 100M 1> /dev/null 2>&1
        ...
        partprobe $LDEV 1> /dev/null 2>&1
        mkfs.vfat "${LDEV}p1" -n BOOT 1> /dev/null 2>&1
        mkfs.ext4 -O ^metadata_csum,^64bit ${LDEV}p2 -L ROOT 1> /dev/null 2>&1
